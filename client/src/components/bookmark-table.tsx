...
import { Search, ExternalLink, Edit, Trash2, ChevronUp, ChevronDown, Save, X } from "lucide-react";
...
export default function BookmarkTable({ }: BookmarkTableProps) {
  ...
  const { data: bookmarkData, isLoading } = useQuery<BookmarkResponse>({
    queryKey: [
      '/api/bookmarks',
      { search: debouncedSearch, category: selectedCategory, page: currentPage }
    ],
    queryFn: async () => {
  ...
  const handleEdit = (id: number, title: string, url: string, category: string) => {
                          ...
                          <td className="px-6 py-4">
                            <div className="flex space-x-2">
                              ...
                              {/* <Button
                                ...
                                onClick={() => { handleEdit(bookmark.id, bookmark.title, bookmark.url, bookmark.category || 'Other') }}
                              >
